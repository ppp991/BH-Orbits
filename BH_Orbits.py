...
class KerrMetricFull():
    ...
    def geodesics(self, tau, state):
        """
        Return the geodesics of each dimensional quantity for the Kerr Metric

        Parameters:
        tau: float
            Current proper time value
        state: array
            Array of the current state variables ([t, r, r_dot, theta, theta_dot, phi])

        Returns:
        geodesics: array
            Array of the ODE of each state variable ([dt_dtau, dr_dtau, drdot_dtau, dtheta_dtau, 
            dthetadot_dtua, dphi_dtau])
        """
        ...
        # Derivatives of effective potentials 
...
class InitialConditions():
    ...
    def circular_orbit_sc(self, r0=6.0):
        """
        Returns the initial conditions and the E (energy of the (particle-BH) system) and 
        L (angular momentum of the system) for the circular orbit case
        ...
        E = np.sqrt(V_eff)  # Ensure E^2 = V_eff(r0)
    ...
    def circular_orbit_newt(self, r0=7.0):
        """
        Returns the initial conditions and the E (energy of the (particle-BH) system) and 
        L (angular momentum of the system) for the Newtonian model
    ...
    def plunging_orbit_sc(self, r0=3.5, E_plunge=1.05, L_plunge=3.0):
        """
        Returns the initial conditions and the E (energy of the (particle-BH) system) and 
        L (angular momentum of the system) for the plunging orbit case
        ...
        E: float
            Particle energy (E>1 for plunging)
        L: float
    ...
    ###### PROJECT EXTENSION #######
    def kerr_equatorial(self, r0=6.0, a=0.9):
        """
    ...
    def kerr_3D(self, r0=9.0, a=0.9, Q=10.0, E=0.95, L=3.0, theta0=np.pi/3):
        """
        Returns the initial conditions and the E (energy of the (particle-BH) system) and L (angular momentum of the system)
...
def schwarzschild_to_cartesian(r, phi, z_offset=0.0):
    """
    Function for converting polar --> cartesian
    """
    x = r * np.cos(phi)
    y = r * np.sin(phi)
...
def extract_states(name, states):
    """
    Function to extract the states depending on physical model
    """
...
def initialize_orbit(cfg, init_cond):
    """
    Obtain initial conditions based on the classification of the test particle
    """
...
def make_plunge_condition(M):
    """
    Function to ensure plunging simulation stops at event horizon
    """
...
def classify_sc_orbit(r, rdot, L, M):
    """
    Classify Schwarzschild orbit based on dynamical conditions (E and r).
    Note that these classifications are for the plot that displays all Sc orbits at once
    """
    v_r = rdot[-1]
    v_phi = L / r[-1]**2 # orbital velocity magnitude
    v_squared = v_r**2 + (r[-1]**2 * v_phi**2)
    E_kin = 0.5 * v_squared # Kinetic Energy
    E_total = E_kin - M / r[-1] # Observe total E for escaping classification
    final_r = r[-1] # Observe final radius for plunging classification
    dr = r[-1] - r[-2] # Observe dr for escaping classification

    # Classify test particle orbit
    if final_r <= 2.05:
...
def animate_orbit_2D(x, y, name, black_hole=True, color='tab:blue', filename='orbit.gif', frame_step=10):
    """
    Animate 2D plots of test particle orbits
    """
...
def animate_3D_particles_motion(particle_data, filename="schwarzschild_particles.gif", duration_sec=10):
    """
    Animate 3D plots of test particle orbits
    """
...
def simulate_nbody_schwarzschild(particle_configs, init_cond, M, steps):
    """
    Simulate the orbits of different Schwarzschild test particle cases

    Parameters:
    particle_configs: dict
        Dictionary of particle configurations (name, initial conds, etc.)
    init_cond: tuple
        Instance of the class InitialConditions
    M: float
        Black hole mass
    steps: float
    ...
    # Loop over all particles specified
    for i, cfg in enumerate(particle_configs):
        termination_condition = None # Initialize plunging condition
        T_orbit = 2 * np.pi * np.sqrt(cfg["r0"]**3 / M) # Orbital period
        total_time = 20 * T_orbit # Total simulation time
        dt = total_time / steps # Step size

        # Obtain initial conds for each case
        ...
        # Classify test particle
        dynamic_label = classify_sc_orbit(r, rdot, L, M)
        x = r * np.cos(phi)
        y = r * np.sin(phi)
        z = 0.0 # Same orbital plane for each orbit
    ...
    ax.scatter(0, 0, 0, color='black', s=100, label="Black Hole")
    ax.set_title("N-body Schwarzschild Orbits (RK45)")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
...
def main():
    ...
    # Plot effective potentials for each Sc orbital case
    ...
    # Loop through test particles and obtain their orbit plots
    ...
    # Particle conifgurations for 3D Schwarzschild multi-orbit visualization
    particle_configs = [
        {'type': 'circular',   'r0': 6.0, 'color': 'tab:blue'},
